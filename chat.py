...
prompt = (
    ...
    "4）关联关系的区分需要注意描绘范围的词（形容词、修饰词等），例如：图像1、2分别展示了一辆敌方坦克车辆处于燃烧状态，而文本1显示敌方装甲车被击毁，敌方装甲部队受到打击。范围从“一辆坦克”被击毁到“敌方装甲部队”受到打击，文本1的表达不局限在图片1、2内容上，因此是关联关系。"
    "5）因果关系要注意时间顺序和因果顺序（即到底是文本1为原因还是图片1、图片2为原因）。例如：图像1、2显示是被导弹轰炸“后”的场景-时间“在后”，而文本1显示的是对敌方进行轰炸-时间“在前”，因此文本1是原因，图片1、图片2是结果，图片1-图片2-文本1是因果关系。"
    "6）所有的数据（图片1、图片2、文本1）都是对敌方的记录，不包含本方相关场景、文本的记录。注意图片1、图片2、文本1的主体和形容词的同义词，例如“坦克”与“装甲车”（坦克也属于装甲车）、“草地/森林”与“植被”（草地、森林等也属于植被）等"
)
