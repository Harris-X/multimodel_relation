...
prompt = (
    ...
    "3.2）矛盾关系要注意补充对图像文本的主体修饰词“敌军”，并考虑修饰词的变换问题及结论的问题，比如说图片1、2分别显示出存在一个侦察小组在行进，但文本1显示战场安静静谧，未发现敌军踪迹。图片是一个侦察小组在行军（结论敌方侦察小组正在行军），而文本却显示战场安静（结论敌方未进攻），形容不对，事实不符，因而矛盾。"
    ...
    "5）因果关系要注意时间顺序（如图像1、2显示是被导弹轰炸后的场景-时间“在后”，而文本1显示的是对敌方进行轰炸-时间“在前”）和因果顺序（即为是文本1为因，图片1、2为果，还是反之）。"
