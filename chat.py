...
prompt = (
    ...
    "对于每张图像：识别图像目标主体的行动意图和场景，以及图像中各个主体之间的关系，主体要肯定不含糊，比如敌军人员在持枪在逮捕、坦克在道路上行进等；"
    ...
    "2）关联关系的区分需要注意描绘范围的词（形容词、修饰词等），例如：图像1、2分别展示了一辆敌方坦克车辆处于燃烧状态，而文本1显示敌方装甲车被击毁，敌方装甲部队受到打击。范围从“一辆坦克”被击毁到“敌方装甲部队”受到打击，文本1的表达不局限在图片1、2内容上，因此是关联关系。"
    ...
    "3.2）矛盾关系要注意补充对图像文本的主体修饰词“敌军”，并考虑修饰词的同义词及结论是否一致，例如图片1、2分别显示出存在一个侦察小组在行进，但文本1显示战场安静静谧，未发现敌军踪迹。图片是一个侦察小组在行军（结论敌方侦察小组正在行军），而文本却显示战场安静（结论敌方未进攻），修饰词不对，事实不符，因而矛盾。"
    "4）等价只要求图片对场景内容的意思（如图像1-图像2主体内容相同，场景相同，即为等价）或图片-文本对（图像1-文本1，图像2-文本1）表达的意思相同即可（例如图像1表达的预测是”1辆“坦克在行径，周围有周围有树木、地形，车辆旁有烟雾，而文本1表达是敌方坦克在行进，因此可以判断这段关系是等价关系，此处只要主体内容一致，即为等价关系）；"
    "4.1)对于图片1-图片2-文本1的等价关系的判断，若三对关系都是等价的或者其中2对关系是等价的，即可判断图片1-图片2-文本1之间的关系是等价关系；"
