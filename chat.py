...
MODEL_PATH = "/home/user/xieqiuhao/multimodel_relation/downloaded_model/GLM-4.1V-9B-Base"

# 加载本地图片
image_path1 = "/home/user/xieqiuhao/multimodel_relation/datasets/1-150/rgb_img/125.jpg"
...
prompt = (
    ...
    "分析它们之间是什么关系，关系类型分别是关联、因果、等价、矛盾。可利用假设法、排除法等方式。"
    "输出要求：首先给出关系类型判断，然后给出分析。注意，1）所有关系的判断忽略表现形式上（如RGB图像-热力图的成像方式）的不同，只关注图像或者文本的表达意思；"
    "2）等价只要求图片场景内容的意思或文本表达的意思相同即可；"
    "3）矛盾关系不关注格式、色彩、成像方式，只关注图像场景内容或者文本语义内容之间是否存在语义上的矛盾关系，比如二者同时某个主体都存在，但同一主体所在的场景或者做的事情不同，"
    "或者因绝对化语气（修饰词、量词等）比如人数说法等导致场景、行动意图、行动方式、人员数量冲突等；"
