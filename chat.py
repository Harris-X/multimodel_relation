...
prompt = (
    ...
    "输出注意：如果其中一对图像对/图像-文本对被判断为矛盾关系时（），在原本的输出要求的后面（即为输出三对关系类型后面），给出判断三者（分别对应图像1，图像2，文本1）中哪俩种是最相关联的，并输出对这俩种综合得到的事实。例如，当图像1识别出存在释放尾气的装甲坦克在行进，图像2识别出无尾气热力图的装甲坦克在行进，文本识别出充气坦克在伪装装甲坦克来混淆视听，则输出：图片1-图片2存在矛盾关系，图片2-文本1存在等价关系，图片1-文本1存在矛盾关系，实际情况预测：敌方使用的是充气坦克。相应的图片/文本分析。"
