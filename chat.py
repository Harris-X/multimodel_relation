...
prompt = (
    ...
    "2）关联关系与等价关系需要区分，关联关系需要注意描绘范围的词（形容词、修饰词等），例如：图像1、2分别展示了一辆敌方坦克车辆处于燃烧状态，而文本1显示敌方装甲车被击毁，敌方装甲部队受到打击。范围从“一辆坦克”被击毁到“敌方装甲部队”受到打击，文本1的表达不局限在图片1、2内容上，因此是关联关系。"
    "3）等价只要求图片对场景内容的意思（如图像1-图像2主体内容相同，场景相同，即为等价）或图片-文本对（图像1-文本1，图像2-文本1）表达的意思相同即可（例如图像1表达的预测是”1辆“坦克在行径，周围有周围有树木、地形，车辆旁有烟雾，而文本1表达是敌方坦克在行进，因此可以判断这段关系是等价关系，此处只要主体内容一致，即为等价关系）；"
    "3.1)对于图片1-图片2-文本1的等价关系的判断，若三对关系都是等价的或者其中2对关系是等价的，即可判断图片1-图片2-文本1之间的关系是等价关系；"
    "4）矛盾关系不关注格式、色彩、成像方式，只关注图像场景内容或者文本语义内容之间是否存在语义上的矛盾关系（注意排除同义词，比如草地与植被等），比如二者同时某个主体都存在，但同一主体所在的场景或者做的事情不同，"
    "4.1）矛盾关系要注意主体的数量问题，比如图片1显示只有2辆坦克，图片2也显示只有2辆坦克，而文本却说只有1辆坦克。或者因绝对化语气（形容词、修饰词、量词等）比如人数量词等导致场景变换、行动意图、行动方式、与人员数量冲突等；"
