...
prompt = (
    ...
    "现分析它们之间（图像1-图像2，图像1-文本1，图像2-文本1）是什么关系，关系类型分别是关联、因果、等价、矛盾。"
    "输出要求：首先给出三对关系类型，然后给出分析，分析过程首先要说明图片/文本的具体内容，然后分析之间的关系。"
    "输出注意：如果其中一对图像对/图像-文本对被判断为矛盾关系时（不包括其他的关系），在原本的输出要求的后面（即为输出三对关系类型后面），输出对三者关系（分别对应图像1，图像2，文本1）中哪俩种是最相关联的判断，并接着回答的开头输出对这俩种综合得到的事实。例如，当图像1识别出存在释放尾气的装甲坦克在行进，图像2识别出无尾气热力图的装甲坦克在行进，文本识别出充气坦克在伪装装甲坦克来混淆视听，则输出：图片1-图片2存在矛盾关系，图片2-文本1存在等价关系，图片1-文本1存在矛盾关系，实际情况预测：敌方使用的是充气坦克。相应的图片/文本分析。"
    "关系判断时注意，1）所有关系的判断忽略表现形式上（如RGB图像-热力图的成像方式）的不同，只关注图像或者文本的表达意思；"
    "2）等价只要求图片对场景内容的意思（如图像1-图像2主体内容相同，场景相同，即为等价）或图片-文本对（图像1-文本1，图像2-文本1）表达的意思相同即可；"
...
# 可选文本；如无则仅图片+提示词
text = None
text = "密级：秘密  等级：紧急  时间：2023年5月10日  发报：军情局  收报：前线指挥部  抄送：各相关部门  主题：观察报告  正文：敌方没有坦克在战场上"
