...
# 关系定义
- 等价 (Equivalent): 描述的核心事实、主体和事件完全相同，不存在任何一方对另一方信息的扩展。
  - 判定依据: 若三对关系均为等价，或其中两对为等价，则总体关系可判定为等价。
- 关联 (Associated): 描述的核心事件相关，但在范围、细节或视角上存在差异。例如，一方描述“一辆坦克”，另一方描述“装甲部队”，后者范围更广。
- 因果 (Causal): 一方是原因，另一方是结果，存在明确的时间或逻辑先后顺序。例如，文本描述“我方发起轰炸”，图像展示“轰炸后的场景”。
  - 判定依据: 若有两对关系为因果，一对为等价/关联，则总体关系可判定为因果。
- 矛盾 (Contradictory): 描述的核心事实存在直接冲突。包括但不限于：
...
# 核心分析准则
- 语义优先: 所有判断只关注内容和语义，完全忽略图像的色彩、风格、成像技术（如可见光 vs. 热成像）等表现形式。
- 敌方视角: 所有输入信息（图、文）均是对敌方情况的记录。
- 同义词兼容: 在分析中要考虑到同义词或上下位词（例如：“坦克”与“装甲车”；“草地”与“植被”）。
